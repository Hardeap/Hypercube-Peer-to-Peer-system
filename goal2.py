...
class P2PNode(threading.Thread):
    ...
    def __init__(self, node_id, n, m=None, nodelay=False):
    ...
    def _store_track(self, track=None):
        if track is None:
            track = []
        track.append(self.id)
        P2PNode.tracks.append(track)
...
def init_nodes(n, m=None, nodelay=False):
    global nodes
    ...
    try:
        for node_id in xrange(2**n):
            node = P2PNode(node_id=node_id, n=n, m=m, nodelay=nodelay)
...
def print_hops_average(m=None):
    if m is not None:
        hops = [len(track) - 1 for track in P2PNode.tracks]
...
if __name__ == "__main__":
    ...
    init_nodes(args.n, args.m, args.no_delay)
    wait_stop()
    print_hops_average(args.m)
